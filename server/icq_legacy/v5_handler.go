...
func (h *V5Handler) handleMetaUser(session *LegacySession, addr *net.UDPAddr, pkt *wire.V5ClientPacket) error {
	...
	case wire.ICQLegacyMetaUserFullInfo, wire.ICQLegacyMetaUserFullInfo2:
	...
	default:
		...
		return h.sendMetaAck(session, pkt.SeqNum1, subCommand)
	}
}
...
// handleGetDeps processes the get departments packet (0x03F2)
// This is a pseudo-login to validate credentials before actual login
...
func (h *V5Handler) handleGetDeps(session *LegacySession, addr *net.UDPAddr, pkt *wire.V5ClientPacket) error {
	...
	if len(pkt.Data) < 6 {
		...
		return h.sendV5BadPassword(addr, pkt.SessionID, pkt.UIN, pkt.SeqNum2)
	}
	...
	if offset+int(pwdLen) > len(pkt.Data) {
		return h.sendV5BadPassword(addr, pkt.SessionID, uin, pkt.SeqNum2)
	}
	...
	if err != nil || !valid {
		h.logger.Info("V5 getdeps FAILED - invalid credentials", "uin", uin, "err", err)
		return h.sendV5BadPassword(addr, pkt.SessionID, uin, pkt.SeqNum2)
	}
	...
	// Send ACK first
	h.sendV5AckToAddr(addr, pkt.SessionID, uin, pkt.SeqNum1, pkt.SeqNum2)

	// Send depslist response (V3 format packet!)
	// From iserverd v5_send_depslist() - this is a V3 format packet
	return h.sendV5DepsListReply(addr, uin, pkt.SeqNum2)
}
...
func (h *V5Handler) handleContactList(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	// Parse contact list - format: COUNT(1) + UIN(4)*COUNT
	if len(pkt.Data) < 1 {
		return h.sendV5ContactListDone(session, pkt.SeqNum2)
	}

	count := int(pkt.Data[0])
	offset := 1

	contacts := make([]uint32, 0, count)
	for i := 0; i < count && offset+4 <= len(pkt.Data); i++ {
		contactUIN := binary.LittleEndian.Uint32(pkt.Data[offset : offset+4])
	...
	for _, contact := range onlineContacts {
		if h.dispatcher != nil {
			...
			h.sendV5UserOnline(session, contact.UIN, contact.GetStatus())
		}
	}
...
func (h *V5Handler) handleSetStatus(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	// Parse status - format: STATUS(4)
	// From iserverd v5_process_status():
	...
	h.logger.Info("V5 status change",
		...
		"new_status", fmt.Sprintf("0x%08X", status),
	)

	// Broadcast status change to all contacts who have this user in their list
	// From iserverd v5_process_status(): broadcast_status(user, old_status)
	h.broadcastStatusChange(session, status)
...
func (h *V5Handler) broadcastStatusChange(session *LegacySession, newStatus uint32) {
	if h.dispatcher == nil {
		return
	}
...
func (h *V5Handler) handleMessage(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	// Parse message header: TARGET_UIN(4) + MSG_TYPE(2) + MSG_LEN(2) + MSG
	var targetUIN uint32
	var msgType uint16
	var msgText string
	if len(pkt.Data) >= 8 {
		targetUIN = binary.LittleEndian.Uint32(pkt.Data[0:4])
		msgType = binary.LittleEndian.Uint16(pkt.Data[4:6])
		msgLen := binary.LittleEndian.Uint16(pkt.Data[6:8])
		if len(pkt.Data) >= 8+int(msgLen) {
			msgText = string(pkt.Data[8 : 8+msgLen])
		}
	}

	h.logger.Info("V5 message",
		"from", session.UIN,
		"to", targetUIN,
	...
	// Use dispatcher for cross-protocol messaging (V5 -> V3 or V5 -> V5)
	...
	if targetSession != nil {
		if h.dispatcher != nil {
			...
			h.dispatcher.SendOnlineMessage(targetSession, session.UIN, msgType, msgText)
		} else {
			// Fallback to V5 format if dispatcher not set
			h.sendOnlineMessage(targetSession, session.UIN, msgType, msgText)
		}
		...
		// From iserverd v5_process_sysmsg(): when target is offline, store in database
		ctx := context.Background()
		if err := h.service.SaveOfflineMessage(ctx, session.UIN, targetUIN, msgType, msgText); err != nil {
...
func (h *V5Handler) handleUserAdd(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	if len(pkt.Data) < 4 {
		h.logger.Info("V5 user add - data too short", "uin", session.UIN)
		return nil
	}
	...
	targetSession := h.sessions.GetSession(targetUIN)
	targetOnline := targetSession != nil
	...
	if targetOnline {
		h.sendV5UserOnline(session, targetUIN, targetSession.GetStatus())
	}
...
func (h *V5Handler) handleLogin(session *LegacySession, addr *net.UDPAddr, pkt *wire.V5ClientPacket) error {
	...
	// Send ACK first (iserverd does this immediately)
	h.sendV5AckToAddr(addr, pkt.SessionID, pkt.UIN, pkt.SeqNum1, pkt.SeqNum2)

	// V5 login data format (from iserverd):
	// JUNK(4) + TCP_PORT(4) + PWD_LEN(2) + PASSWORD + JUNK(4) + INT_IP(4) + DC_TYPE(1) + STATUS(2) + ESTATUS(2) + TCPVER(4) + ...
	if len(pkt.Data) < 10 {
	...
	if offset+4+4+1+2+2+2 <= len(pkt.Data) {
		offset += 4 // skip junk
		internalIP = binary.LittleEndian.Uint32(pkt.Data[offset : offset+4])
		offset += 4
		dcType = pkt.Data[offset]
		offset++
		status = binary.LittleEndian.Uint16(pkt.Data[offset : offset+2])
		offset += 2
		estatus = binary.LittleEndian.Uint16(pkt.Data[offset : offset+2])
		offset += 2
		tcpVersion = binary.LittleEndian.Uint16(pkt.Data[offset : offset+2])
	}
	...
	h.logger.Info("V5 login attempt",
		...
		"estatus", fmt.Sprintf("0x%04X", estatus),
		"tcp_version", tcpVersion,
		"data_len", len(pkt.Data),
	)

	// Validate credentials
	valid, err := h.service.ValidateCredentials(ctx, pkt.UIN, password)
	if err != nil || !valid {
		h.logger.Info("V5 login FAILED - invalid credentials", "uin", pkt.UIN, "err", err)
		return h.sendV5BadPassword(addr, pkt.SessionID, pkt.UIN, pkt.SeqNum2)
	}
	...
	newSession.SetStatus(uint32(status) | uint32(estatus)<<16)

	// Store direct connection info for peer-to-peer
	newSession.SetDirectConnectionInfo(port, internalIP, tcpVersion, dcType)

	// Send login success (HELLO packet)
	if err := h.sendV5LoginReply(newSession, pkt.SeqNum2); err != nil {
		return err
	}
...
func (h *V5Handler) handleMetaSetBasic(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetBasicAck)
}
...
func (h *V5Handler) handleMetaSetWork(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetWorkAck)
}
...
func (h *V5Handler) handleMetaSetMore(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetMoreAck)
}
...
func (h *V5Handler) handleMetaSetAbout(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetAboutAck)
}
...
func (h *V5Handler) handleMetaSetInterests(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetInterestsAck)
}
...
func (h *V5Handler) handleMetaSetAffiliations(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetAffilAck)
}
...
func (h *V5Handler) handleMetaSetSecurity(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetSecureAck)
}
...
func (h *V5Handler) handleMetaSetPassword(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	if len(data) < 2 {
		...
		return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetPassAck)
	}
...
func (h *V5Handler) handleMetaSetHPCat(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaSetHPCatAck)
}
...
func (h *V5Handler) handleMetaUnregister(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	if len(data) < 6 {
		...
		return h.sendMetaUnregAck(session, pkt.SeqNum1, false)
	}
	...
	return h.sendMetaUnregAck(session, pkt.SeqNum1, true)
}
...
func (h *V5Handler) handleMetaUserFullInfo(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	if len(data) < 4 {
		...
		return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaUserInfo)
	}

	ctx := context.Background()
	targetUIN := binary.LittleEndian.Uint32(data[0:4])
	h.logger.Info("META user full info request",
		"uin", pkt.UIN,
		"target_uin", targetUIN,
	)

	// Get user info and send response
	info, err := h.service.GetUserInfo(ctx, targetUIN)
	if err != nil {
		h.logger.Info("META user full info - error", "target_uin", targetUIN, "err", err)
		return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaUserInfo)
	}
	if info == nil {
		h.logger.Info("META user full info - NOT FOUND", "target_uin", targetUIN)
		return h.sendMetaAck(session, pkt.SeqNum1, wire.ICQLegacySrvMetaUserInfo)
	}

	h.logger.Info("META user full info - FOUND",
		"target_uin", targetUIN,
		"nickname", info.Nickname,
	)

	// Send user info response
	return h.sendMetaUserInfo(session, pkt.SeqNum1, info)
}

func (h *V5Handler) handleMetaUserInfo(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	return h.handleMetaUserFullInfo(session, pkt, data)
}

func (h *V5Handler) handleMetaSearchName(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	h.logger.Info("META search by name", "uin", pkt.UIN, "data_len", len(data), "status", "not_implemented")
	// TODO: Parse search parameters and search
	return h.sendMetaSearchEnd(session, pkt.SeqNum2)
}
...
func (h *V5Handler) handleMetaSearchUIN(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	...
	// Send search result
	h.sendMetaSearchResult(session, pkt.SeqNum2, result, true)
	return nil
}

func (h *V5Handler) handleMetaSearchEmail(session *LegacySession, pkt *wire.V5ClientPacket, data []byte) error {
	h.logger.Info("META search by email", "uin", pkt.UIN, "data_len", len(data), "status", "not_implemented")
	// TODO: Parse email and search
	return h.sendMetaSearchEnd(session, pkt.SeqNum2)
}
...
// sendV5DepsListReply sends the departments list response (V3 format!)
// From iserverd v5_send_depslist() - this is a V3 format packet
func (h *V5Handler) sendV5DepsListReply(addr *net.UDPAddr, uin uint32, seq2 uint16) error {
	...
	// Command is 0x0032 (ICQ_CMDxSND_USERxDepsList)
	buf := make([]byte, 16)
...
func (h *V5Handler) sendMetaAck(session *LegacySession, seqNum uint16, subCommand uint16) error {
	if session == nil {
		return nil
	}
	...
	pkt := &wire.V5ServerPacket{
		...
		Command:   wire.ICQLegacySrvMetaUser,
		SeqNum1:   seqNum,
		SeqNum2:   session.NextServerSeqNum(),
		UIN:       session.UIN,
		Data:      metaData,
	}
...
func (h *V5Handler) sendMetaUnregAck(session *LegacySession, seqNum uint16, success bool) error {
	if session == nil {
		return nil
	}
...
//
// Packet format (from iserverd make_packet.cpp):
//   Header: VERSION(2) + GAP(1) + SESSION_ID(4) + COMMAND(2) + SEQ1(2) + SEQ2(2) + UIN(4) + JUNK(4)
//   Data: TARGET_UIN(4) + CITY_LEN(2) + CITY + COUNTRY(2) + 0x01(1) + STATE_LEN(2) + STATE +
//         AGE(2) + GENDER(1) + PHONE_LEN(2) + PHONE + HPAGE_LEN(2) + HPAGE + NOTES_LEN(2) + NOTES
func (h *V5Handler) sendV5OldStyleInfoExt(session *LegacySession, targetUIN uint32, user *state.User) error {
	...
	// Build data: UIN(4) + CITY_LEN(2) + CITY + COUNTRY(2) + 0x01(1) + STATE_LEN(2) + STATE +
	//             AGE(2) + GENDER(1) + PHONE_LEN(2) + PHONE + HPAGE_LEN(2) + HPAGE + NOTES_LEN(2) + NOTES
	buf := new(bytes.Buffer)
	...
	buf.WriteByte(0x01)                                 // unknown byte (from iserverd: "I don't know what is it")
	...
	writeLegacyString(buf, notes)                       // notes (length-prefixed with null terminator)
...
func (h *V5Handler) handleDirectWhiteSearch(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	h.logger.Info("V5 direct white pages search", "uin", session.UIN, "data_len", len(pkt.Data), "status", "not_implemented")
...
func (h *V5Handler) handleDirectNameSearch(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	h.logger.Info("V5 direct name search", "uin", session.UIN, "data_len", len(pkt.Data), "status", "not_implemented")
...
func (h *V5Handler) handleDirectEmailSearch(session *LegacySession, pkt *wire.V5ClientPacket) error {
	...
	h.logger.Info("V5 direct email search", "uin", session.UIN, "data_len", len(pkt.Data), "status", "not_implemented")
