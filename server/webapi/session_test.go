...
func TestSession_PublishesMoodOnPresence(t *testing.T) {
	...
	t.Run("a mood capability yields the mood icon URL", func(t *testing.T) {
		...
		assert.Equal(t, "http://host/mood?id="+moodIconID("0icqmood4"), got.MoodIcon)
	})

	t.Run("a placeholder capability resolves too", func(t *testing.T) {
		got := arrived(newSession(), capBytes(capMoodOnTheWay), false)
		assert.Equal(t, "http://host/mood?id="+moodIconID("0icqmood83"), got.MoodIcon)
	})
	...
	t.Run("a shared capability resolves to the canonical mood", func(t *testing.T) {
		...
		assert.Equal(t, "http://host/mood?id="+moodIconID("0icqmood81"), got.MoodIcon)
	})
