...
func TestNewAccountCreator(t *testing.T) {
	tests := []struct {
		...
		password             string
		insertUserErr        error
		insertFeedbagErr     error
		wantErr              error
		validateUser         func(*testing.T, User)
		validateReturnedUser func(*testing.T, User)
		validateFeedbag      func(*testing.T, IdentScreenName, []wire.FeedbagItem)
	}{
		{
			...
			validateReturnedUser: func(t *testing.T, u User) {
				assert.Equal(t, DisplayScreenName("TestUser123"), u.DisplayScreenName)
				assert.Equal(t, NewIdentScreenName("testuser123"), u.IdentScreenName)
				assert.False(t, u.IsICQ)
				assert.NotEmpty(t, u.AuthKey)
			...
			validateFeedbag: func(t *testing.T, ident IdentScreenName, items []wire.FeedbagItem) {
				assert.Equal(t, NewIdentScreenName("testuser123"), ident)
		...
		},
		{
			...
			validateFeedbag: func(t *testing.T, ident IdentScreenName, items []wire.FeedbagItem) {
				assert.Equal(t, NewIdentScreenName("12345678"), ident)
				assert.Empty(t, items)
			},
		},
		{
			...
			validateFeedbag: func(t *testing.T, ident IdentScreenName, items []wire.FeedbagItem) {
				assert.Equal(t, NewIdentScreenName("test user 123"), ident)
				assert.Empty(t, items)
			},
		},
		{
		...
		{
			name:             "insertFeedbag error",
			screenName:       "TestUser123",
			password:         "validpass123",
			insertFeedbagErr: assert.AnError,
			wantErr:          assert.AnError,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var capturedIdent IdentScreenName
			var capturedFeedbagItems []wire.FeedbagItem
			...
			insertFeedbag := func(ctx context.Context, screenName IdentScreenName, items []wire.FeedbagItem) error {
				capturedIdent = screenName
				capturedFeedbagItems = items
				return tt.insertFeedbagErr
			}

			createAccount := NewAccountCreator(insertUser, insertFeedbag)
			returnedUser, err := createAccount(context.Background(), tt.screenName, tt.password)
			...
			if tt.validateReturnedUser != nil {
				tt.validateReturnedUser(t, returnedUser)
			}

			if tt.validateFeedbag != nil {
				tt.validateFeedbag(t, capturedIdent, capturedFeedbagItems)
			}
		})
	}
}
