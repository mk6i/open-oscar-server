...
func TestOServiceService_SetUserInfoFields(t *testing.T) {
	cases := []struct {
		...
		{
			// Empty text is how the web clients clear a status message.
			name:     "clear status message with empty text",
		...
		{
			// An OSCAR client clears it the way it clears a buddy icon, by dropping
			// the data flag.
			name:     "clear status message with a non-data BART item",
			instance: newTestInstance("me", sessOptStatus(newTestStatusBARTID(t, "old"))),
			inputSNAC: wire.SNACMessage{
				Frame: wire.SNACFrame{
					RequestID: 1234,
					...
					TLVRestBlock: wire.TLVRestBlock{
						TLVList: wire.TLVList{
							wire.NewTLVBE(wire.OServiceUserInfoBARTInfo, wire.BARTID{
								...
								BARTInfo: wire.BARTInfo{Flags: wire.BARTFlagsKnown},
							}),
						},
					},
				},
			...
			expectOutput: wire.SNACMessage{
				...
				Body: func(val any) bool {
					...
					// The cleared status must stop riding along in the BART TLV.
					return !snac.UserInfo[0].HasTag(wire.OServiceUserInfoBARTInfo)
				},
			},
			mockParams: mockParams{
				buddyBroadcasterParams: buddyBroadcasterParams{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// The session's other instances get the same update, unsolicited, so it
			// carries the server request ID rather than the caller's.
